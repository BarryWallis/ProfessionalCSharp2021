...
public class Vector : IFormattable, IEnumerable<double>
{
    ...
    public double this[uint i] => i switch
    {
        0 => X,
        1 => Y,
        2 => Z,
    ...
    {
        private readonly Vector _theVector;      // Vector object that this enumerator refers to 
        private int _location;   // which element of _theVector the enumerator is currently referring to 
        ...
        double IEnumerator<double>.Current => _location is < 0 or > 2
                    ? throw new InvalidOperationException(
                        "The enumerator is either before the first element or " +
                        "after the last element of the Vector")
                    : _theVector[(uint)_location];

        [LastModified("2020/12/19", "Change to use Lambda")]
