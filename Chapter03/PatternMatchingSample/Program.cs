...
TrafficLight previousLight = Red;
...
#region Methods
static (TrafficLight Current, TrafficLight Previous) NextLightUsingTuples(TrafficLight current,
...
static TrafficLightState NextLightUsingRecords(TrafficLightState trafficLightState)
    => trafficLightState switch
    {
        ...
        { CurrentLight: GreenBlink, BlinkCount: < 3 }
            => trafficLightState with { BlinkCount = trafficLightState.BlinkCount + 1 },
...
public record TrafficLightState(TrafficLight CurrentLight, TrafficLight PreviousLight, int Milliseconds,
                                int BlinkCount = 0);
